...
export class RequestStore {
  ...
  readonly pendingRequests = this._requests.select(requests =>
    requests.filter(x => x.status === RequestStatus.Pending)
  )
  readonly newRequests = this._requests.select(requests =>
    requests.filter(x => x.status === RequestStatus.New)
  )
  readonly cachedRequests = this._requests.select(requests =>
    requests.filter(x => x.status === RequestStatus.Cached)
  )
  readonly completedRequests = this._requests.select(requests =>
    requests.filter(x => x.status !== RequestStatus.Pending)
  )
  readonly progress = this._requests.select(
    requests =>
      requests.filter(x => x.status !== RequestStatus.Pending).length /
        requests.length || 0
  )
  readonly cachedProgress = this._requests.select(
    requests =>
      requests.filter(x => x.status === RequestStatus.Cached).length /
        requests.length || 0
  )
  readonly newProgress = this._requests.select(
    requests =>
